...
public class AccountDao {
	...
	public int update(Account account) throws SQLException{
		int rows = 0;
		String sql = "update accoounts set account_balance=? where account_no=";
	...
	public Account selectByPk(int accountNo) throws SQLException{
		...
		String sql = "select * from accounts where accout_no=?";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		...
		if(rs.next()){
			...
			account.setBalance(rs.getInt("account_no"));
